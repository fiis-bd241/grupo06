...
def insert_lote_entrada(cursor): # Inserta lote de entrada
    try:
        ...
        for _ in range(200):
            ...
            print(lotes_dict[id_lote])
            print(f"""
                SELECT e.id_espacio 
                FROM espacio e
                JOIN zona z ON CAST(e.id_espacio AS VARCHAR) LIKE CAST(z.id_zona AS VARCHAR) || '%'
                WHERE z.nombre = '{lotes_dict[id_lote]}';
            """)
            cursor.execute("""
                SELECT e.id_espacio 
                FROM espacio e
...
def insert_lote_salida(cursor): # Inserta lote de salida
    try:
        ...
        lotes_dict = {}
        for lote in lotes:
            id_lote, tipo_lote = lote
            if tipo_lote not in lotes_dict:
                lotes_dict[tipo_lote] = []
            lotes_dict[tipo_lote].append(id_lote)

        # Obtener los identificadores de las áreas de envío
        cursor.execute("SELECT id_area, nombre FROM area WHERE nombre IN ('Corte', 'Confección', 'Acabado');")
        areas = cursor.fetchall()
        areas_dict = {area[1]: area[0] for area in areas}

        for _ in range(200):
            fecha_salida = fake.date_time_between(start_date='-1y', end_date='now', tzinfo=None)
            area_envio = random.choice(['Corte', 'Confección', 'Acabado'])
            id_area_envio = areas_dict[area_envio]
            if area_envio == 'Corte':
                tipo_lote = 'Materia Prima'
            elif area_envio == 'Confección':
                tipo_lote = 'Corte'
            else:
                tipo_lote = 'Confección'
            
            
            id_lote = random.choice(lotes_dict[tipo_lote])
